...
inline std::string getVersion() {
	//Note: glGetString() MUST be called after getting GL context - otherwise, it will segfault.
	int major, minor, revision;
	glfwGetGLVersion(&major, &minor, &revision);
	return ("OpenGL version: " + toString(major) + "." +  toString(minor) + " rev. " + toString(revision)
